...
function BookingCard({ singlePlace }) {
  ...
  let totalNoOfDays = 0;
  if (checkIn && checkOut) {
    totalNoOfDays = differenceInCalendarDays(
      new Date(checkOut),
      new Date(checkIn)
    );
  }
    ...
    <div className="bg-white shadow p-4 rounded-2xl">
      ...
      <div className="border rounded-2xl mt-4">
        ...
        <div className="py-3 px-4 border-t">
          <label>Guests:</label>
          <input
            className="outline-none border px-2 rounded-2xl ml-2"
            min={1}
            type="number"
            value={noOfGuests}
            ...
            <div className="py-3 px-4 flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <h2 className="text-lg underline">
                  ₹{singlePlace?.price} x {totalNoOfDays} nights
                </h2>
                <h2 className="text-lg">
                  ₹{totalNoOfDays * singlePlace?.price}
                </h2>
              </div>
              <div className="flex items-center justify-between">
      ...
      {loading ? (
        ...
        <button
          onClick={bookPlace}
          className="primary mt-4 flex justify-center gap-8"
        >
          Book Now
          {totalNoOfDays > 0 && (
            <>
